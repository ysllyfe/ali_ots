module AliOts
  class Client
    ...
    def list_search_index(table_name)
      request_meta = AliOts::Metas::ListSearchIndexRequest.new(table_name: table_name)

      status, body, response_status = self.conn.request("ListSearchIndex", request_meta)
      body = AliOts::Metas::ListSearchIndexResponse.decode(body) if status == "OK"
      return status, body, response_status
    end
  end
end